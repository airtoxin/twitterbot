...
def generateMarkovSentence(markov_dict):
    ...
    while True:
        ...
        if back == u"\\end\\":
            ...
            if re.search(r"\w", markov_sentence[-1]): #英単語が繋がらないようにする
                markov_sentence += u" "
...
def generate_reply(user_id):
    markov_dict = loadObject("./datas/markov_dict.dump")
    reply = generateMarkovSentence(markov_dict)
    reply = re.sub(u"^RT ", u"", reply) #先頭のRTを削除
    reply = re.sub(u"@[0-9A-Za-z_]*", u"", reply) #リプライの表示を削除
    twitpic = generate_twitpic()
    reply = m.sub(twitpic, reply)
    reply_user_id = "@%s " % user_id
    reply = reply_user_id + reply
    return reply
    ...
    if reps != []:
        for rep in reps:
            ...
            if rep_user_id == "airtoxinbotbot":
                continue
            rep_id = rep.id
            replie = generate_reply(rep_user_id)
            status = api.PostUpdate(replie, in_reply_to_status_id=rep_id)
            try:
